#!/usr/bin/env python3
"""Produce deterministic, review-only Textstat evidence from a canonical PDF."""
...
def rounded(value: int | float) -> int | float:
    """Keep integer counts exact and stabilize floating-point presentation."""
...
def build_report(text: str, source_name: str) -> dict[str, Any]:
    ...
    try:
        ...
        raise SystemExit(
            "missing pinned Textstat corpus: run `make setup-tools` first"
        ) from error
    ...
    return {
        "schema_version": "arxiv-paper-template.readability.v1",
        "status": "review_only",
        "source": source_name,
        "language": "en_US",
...
def main() -> None:
    ...
    if len(sys.argv) != 3:
        raise SystemExit("usage: audit_readability.py INPUT.pdf OUTPUT.json")
    pdf = Path(sys.argv[1])
